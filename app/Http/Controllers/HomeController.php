...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            $visitors = SmVisitor::whereMonth('date', '=', date('m'))
            ...
            $attendanceData = SmStudent::join('sm_student_attendances', 'sm_student_attendances.student_id', '=', 'sm_students.id')
                    ...
                    ->select('attendance_date','attendance_type','class_name')
                    ->orderBy('sm_students.class_id', 'asc')
            ...
            $all_students = SmStudent::where('active_status', 1)
                ->where('school_id', $school_id)->get();
            $data =[
                'studentPerClass' => SmStudent::join('sm_classes', 'sm_classes.id', '=', 'sm_students.class_id')
                    ->where('sm_students.active_status', 1)
                    ->where('sm_students.academic_id', getAcademicId())
                    ->where('sm_students.school_id', Auth::user()->school_id)
                    ->select('sm_classes.class_name', DB::raw('count(*) as total'))
                    ->groupBy('sm_classes.class_name')
                    ->orderBy('sm_classes.id', 'asc')
                    ->get(),
                    
                'studentPerGender' => SmStudent::join('sm_base_setups', 'sm_base_setups.id', '=', 'sm_students.gender_id')
                    ->where('sm_students.active_status', 1)
                    ->where('sm_students.academic_id', getAcademicId())
                    ->where('sm_students.school_id', Auth::user()->school_id)
                    ->select('sm_base_setups.base_setup_name', DB::raw('count(*) as total'))
                    ->groupBy('sm_base_setups.base_setup_name')
                    ->orderBy('sm_base_setups.id', 'asc')
                    ->get(),

                'studentPerReligion' => SmStudent::join('sm_base_setups', 'sm_base_setups.id', '=', 'sm_students.religion_id')
                    ->where('sm_students.active_status', 1)
                    ->where('sm_students.academic_id', getAcademicId())
                    ->where('sm_students.school_id', Auth::user()->school_id)
                    ->select('sm_base_setups.base_setup_name', DB::raw('count(*) as total'))
                ...
                'studentPerBloodGroup' => SmStudent::join('sm_base_setups', 'sm_base_setups.id', '=', 'sm_students.bloodgroup_id')
                    ->where('sm_students.active_status', 1)
                    ->where('sm_students.academic_id', getAcademicId())
                    ->where('sm_students.school_id', Auth::user()->school_id)
                    ->select('sm_base_setups.base_setup_name', DB::raw('count(*) as total'))
                ...
                'studentPerAgeGroup' => SmStudent::where('active_status', 1)
                    ->where('academic_id', getAcademicId())
                    ->where('school_id', Auth::user()->school_id)
                    ->select(DB::raw('floor(datediff(curdate(), date_of_birth)/365) as CalculatedAge') , DB::raw('count(*) as total'))
                    ->groupBy('CalculatedAge')
                    ->orderBy('CalculatedAge', 'asc')
                    ->get(),

                'studentPerAdmissionYear' => SmStudent::where('active_status', 1)
                    ->where('academic_id', getAcademicId())
                    ->where('school_id', Auth::user()->school_id)
                    ->select(DB::raw('YEAR(admission_date) as year'), DB::raw('count(*) as total'))
                    ->groupBy('year')
                    ->orderBy('year', 'asc')
                    ->get(),

                'studentPerHeightGroup' => SmStudent::where('active_status', 1)
                    ->where('academic_id', getAcademicId())
                    ->where('school_id', Auth::user()->school_id)
                    ->select('height', DB::raw('count(*) as total'))
                    ->groupBy('height')
                    ->orderBy('height', 'asc')
                    ->get(),

                'studentPerWeightGroup' => SmStudent::where('active_status', 1)
                    ->where('academic_id', getAcademicId())
                    ->where('school_id', Auth::user()->school_id)
                    ->select('weight', DB::raw('count(*) as total'))
                    ->groupBy('weight')
                    ->orderBy('weight', 'asc')
                    ->get(),
